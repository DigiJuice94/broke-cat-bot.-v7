...
import {discoverCandidates,pairForToken,refreshPair} from './dexscreener.mjs';
...
const server=http.createServer(async(req,res)=>{if(req.url==='/health'){try{if(isLive)lastWallet=await walletEquitySnapshot(state)}catch{}const payload=isLive?{ok:true,version:'v9.4',mode:'LIVE',wallet:walletAddress(),sol:lastWallet?.sol??null,totalWalletUsd:lastWallet?.totalWalletUsd??null,dailyPnl:state.dailyPnl,realizedPnl:state.realizedPnl,hasPosition:Boolean(state.position),moonBags:state.moonBags?.length||0,xPosting:xReady(),xIntel:{...intelStatus(),stream:streamStatus()},runnerFeeds:runnerFeedStatus(),scans,lastScanAt,lastError,stateFile:liveStateFilePath()}:{ok:true,version:'v9.4',mode:'paper',cash:state.cash,equity:equityUsd(state),dailyPnl:state.dailyPnl,hasPosition:Boolean(state.position),moonBags:state.moonBags?.length||0,xPosting:xReady(),xIntel:{...intelStatus(),stream:streamStatus()},runnerFeeds:runnerFeedStatus(),scans,lastScanAt,lastError,stateFile:stateFilePath()};res.writeHead(200,{'content-type':'application/json'});res.end(JSON.stringify(payload));return}res.writeHead(200,{'content-type':'text/plain'});res.end(`Broke Cat Bot V9.4 ${isLive?'LIVE MODE':'PAPER MODE'} 🐱`)});
...
if(isLive){lastWallet=await assertLiveFunding();await alert(`🐱 Broke Cat V9.4 STARTED | 🔴 LIVE MONEY | wallet ${walletAddress()} | SOL ${lastWallet.sol.toFixed(6)} (~$${lastWallet.solValueUsd.toFixed(2)}) | sizing ${config.positionSizingMode} | min score ${config.minScore} | X ${xReady()?'ON':'OFF'} | X intel ${intelStatus().enabled?'ON':'OFF'}`)}else await alert(`🐱 Broke Cat V9.4 started | PAPER MODE | bankroll $${state.cash.toFixed(2)} | min score ${config.minScore} | X ${xReady()?'ON':'OFF'} | X intel ${intelStatus().enabled?'ON':'OFF'}`);
