...
export function buyPost({mode='PAPER',symbol,sizeUsd,score,marketCap,risk,cash}){const bundle=Number.isFinite(risk?.estimatedLinkedSupplyPct)?`${risk.estimatedLinkedSupplyPct.toFixed(1)}% est. linked`:String(risk?.bundleRisk||'unknown');return `🐱 BROKE CAT ${mode} BUY\n$${symbol} | $${sizeUsd.toFixed(2)}\nScore ${score}/100 | MC $${Math.round(marketCap).toLocaleString()}\nBundle ${bundle}\nUSDC $${cash.toFixed(2)}\n\nAutomated experiment. High risk.`}
export function sellPost({mode='PAPER',symbol,reason,pnlUsd,cash}){const sign=pnlUsd>=0?'+':'';return `🐱 BROKE CAT ${mode} EXIT\n$${symbol} | ${reason}\nP&L ${sign}$${pnlUsd.toFixed(2)}\nUSDC $${cash.toFixed(2)}\n\nAutomated experiment. High risk.`}
export function dailyPost({mode='PAPER',cash,realizedPnl,dailyPnl,wins,losses,totalTrades}){const t=realizedPnl>=0?'+':'',d=dailyPnl>=0?'+':'';return `🐱 BROKE CAT ${mode} DAILY\nUSDC $${cash.toFixed(2)}\nTotal P&L ${t}$${realizedPnl.toFixed(2)} | Today ${d}$${dailyPnl.toFixed(2)}\nTrades ${totalTrades} | W ${wins} / L ${losses}\n\nAutomated experiment. High risk.`}
